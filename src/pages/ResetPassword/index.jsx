...
import { PASSWORD_SET, RESET_PASSWORD } from 'apollo/mutation';
import Header from 'components/Header';
import Footer from 'components/FooterLandingPage';
import { Column, Row, SubHeading } from 'globalStyles';
    ...
    try {
      ...
      if (errors) {
        ...
        if (errorStr.find((err) => err.code === 'invalid_token')) {
          setError({
            loginErr: <>Password update link has invalid. Please request a new link on App</>,
          });
          return;
        }
        if (errorStr.find((err) => err.code === 'expired_token')) {
          setError({
            loginErr: <>Password update link has expired. Please request a new link on App</>,
          });
          return;
        }
        throw new Error(`Password update Failed, ${errorStr.map((err) => err.message).join('\n')}`);
      }
      if (success) {
        history.push({
          pathname: '/success',
